...
def load_user_item(path: str) -> pd.DataFrame:
    '''
    user_id: 1, 2, 3, ...
...
def load_item_attr(path: str) -> pd.DataFrame:
    '''
    item_id: 1, 2, 3, ...
...
def neg_sampling(pos_list: list, total_size: int, ns: int) -> list:
    '''
...
def shuffle(n_user: int, *data: np.ndarray) -> list:
    # shuffle arrays
...
def one_hot_encoding(attr_lists: np.ndarray, 
                     max_item_len: int, 
                     n_attr: int) -> np.ndarray:
    '''
    ...
    for attr_list in attr_lists:
        encoded = MLB.fit_transform(attr_list)[-max_item_len:] # discard attributes corresponding discarded items
        if max_item_len > 1:
            encoded = np.pad(encoded, ((max_item_len - len(encoded), 0), (0, 0)))
        res.append(encoded)
    return np.squeeze(np.stack(res))

def make_mask(item_list: list, n_item: int) -> Tuple[np.ndarray]:
    '''
...
def make_segment(item_list: list, n_item: int) -> Tuple[np.ndarray]:
    '''
    if item_list = [1, 2, 3, 4], then
    masked = [1, 0, 0, 4]
    pos_seg = [0, 2, 3, 0]
