...
class MultiHeadAttention(tf.keras.layers.Layer):
    ...
    def call(self, inputs, mask, return_attention_weight=False, training=False):
        ...
        concat_attention = tf.reshape(
            scaled_attention, (batch_size, -1, self.latent_dim)
    ...
    def build(self, input_shape):
        for _ in range(self.n_layer):
            self.layer_list.append(
                [
                    MultiHeadAttention(
                        input_shape[-1], self.n_head, self.dropout_rate, epsilon=self.epsilon
                    ),
                    FeedForwardNetwork(self.n_ffn_unit, self.dropout_rate, self.epsilon),
                ]
            )
