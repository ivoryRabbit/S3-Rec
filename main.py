...
class finetune:
    def run(self, user_item_fname, use_pretrained, verbose):
        ...
        model = S3Rec(n_user, n_item, max_item_len, latent_dim,
        ...
        model_checkpoint = ModelCheckpoint(checkpoint, monitor = 'val_NDCG@10', mode = 'max', save_best_only = False)

        print('start finetuning...')
        model.fit(train_gen, epochs = epochs, steps_per_epoch = steps_per_epoch, 
                  validation_data = valid_gen, validation_steps = steps_per_epoch // 10, # sampling 10% of dataset to validate
                  callbacks = [model_checkpoint], verbose = verbose)
