...
from utils import (
    neg_sampling, shuffle, batch_slice, one_hot_encoding, 
    make_mask, make_segment
)

def train_generator(user_item: pd.DataFrame, 
                    n_user: int, 
                    n_item: int, 
                    max_item_len: int, 
...
def valid_generator(user_item: pd.DataFrame, 
                    n_user: int, 
                    n_item: int,
                    max_item_len: int, 
                    batch_size: int) -> Tuple[np.ndarray]:
...
def test_generator(user_item: pd.DataFrame, 
                   n_user: int, 
                   n_item: int,
                   max_item_len: int, 
                   batch_size: int) -> Tuple[np.ndarray]:
...
def AAP_generator(item_attr: pd.DataFrame, 
                  n_item: int, 
                  n_attr: int, 
                  batch_size: int) -> Tuple[np.ndarray]:
    '''
...
def mip_train_data(user_item: pd.DataFrame, n_item: int) -> pd.DataFrame:
    ...
    temp_df[['masked', 'pos_targ', 'neg_targ']] = temp_df.apply(
        lambda x: make_mask(x.item_list, n_item), 
        axis = 1, 
        result_type = 'expand'
    )
    return temp_df.drop(columns = ['user_id', 'item_list'])

def MIP_generator(user_item: pd.DataFrame, 
                  n_user: int, 
                  n_item: int,
                  max_item_len: int, 
                  batch_size: int) -> Tuple[np.ndarray]:
    ...
    pad_seq = lambda d: pad_sequences(d, max_item_len, padding = 'pre')
    while True:
        train = mip_train_data(user_item, n_item) # initialize sampling for each epoch, but consume too much time..
...
def map_train_data(user_item: pd.DataFrame, item_attr: pd.DataFrame) -> pd.DataFrame:
    ...
    '''
    temp_df = user_item.assign(item_list = user_item.item_list.apply(lambda x: x[:-3]))
    item2attr = {item_attr.at[idx, 'item_id']: item_attr.at[idx, 'attr_list'] for idx in item_attr.index}
    user_attr = temp_df.assign(attr_list = lambda df: df.item_list.apply(lambda x: [item2attr[i] for i in x]))

    return user_attr.drop(columns = ['user_id'])

def MAP_generator(user_item: pd.DataFrame, 
...
def sp_train_data(user_item: pd.DataFrame, n_item: int) -> pd.DataFrame:
    ...
    temp_df[['masked', 'pos_seg', 'neg_seg']] = temp_df.apply(
        lambda x: make_segment(x.item_list, n_item), 
        axis = 1,   
        result_type = 'expand'
    )
    return temp_df.drop(columns = ['user_id', 'item_list'])

def SP_generator(user_item: pd.DataFrame, 
                 n_user: int, 
                 n_item: int, 
                 max_item_len: int, 
                 batch_size: int) -> Tuple[np.ndarray]:
    ...
    pad_seq = lambda d: pad_sequences(d, max_item_len, padding = 'pre')
    while True:
        train = sp_train_data(user_item, n_item) # initialize sampling for each epoch, but consume too much time..
        masked = train.masked.to_numpy()
