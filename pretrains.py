...
from generators import AAP_generator, MIP_generator, MAP_generator, SP_generator
...
class pretrain:
    def run(self, user_item_fname, item_attr_fname, verbose):
        ...
        attr_embedding_layer = tf.keras.layers.Embedding(
            n_attr + 1, latent_dim, mask_zero=True, input_length=max_attr_len, name="attr_embedding"
        )
        ...
        map_train_gen = MAP_generator(
            user_item, item_attr, n_user, n_attr, max_item_len, batch_size
        )
        ...
        for s in range(100):
            ...
            aap_model.fit(
                aap_train_gen, epochs=epochs, steps_per_epoch=item_per_batch, verbose=verbose
            )

            # Masked Attribute Prediction
            mip_model.fit(
                mip_train_gen, epochs=epochs, steps_per_epoch=user_per_batch, verbose=verbose
            )

            # Masked Attribute Prediction
            map_model.fit(
                map_train_gen, epochs=epochs, steps_per_epoch=user_per_batch, verbose=verbose
            )

            # Segment Predict
            sp_model.fit(
                sp_train_gen, epochs=epochs, steps_per_epoch=user_per_batch, verbose=verbose
            )
